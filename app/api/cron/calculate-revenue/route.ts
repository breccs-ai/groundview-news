...
export async function GET(req: NextRequest) {
  ...
  const { data: ads, error: adsErr } = await supabase
    .from('advertisements')
    .select('price_gbp, price_paid, status, paid_at, updated_at')
  ...
  const totalAdRevenue = (ads || []).reduce((sum, a: { price_gbp?: number; price_paid?: number }) => {
    const v = Number(a.price_gbp ?? a.price_paid);
